 ...
 *
 * If {@link auto_start} is set true, the session will be started automatically
...
//			Yii::log($message, CLogger::LEVEL_WARNING, 'system.web.Session');
	 ...
	 * @throws RThink_Exception if the path is not a valid directory
	 */
	public function setSavePath($value)
	{
		if(is_dir($value))
			...
			throw new RThink_Exception('Session.savePath "{path} => '. $value . '" is not a valid directory.');
	}
	...
	public function setCookieMode($value)
	{
		...
		else if($value==='only')
		{
			...
			throw new RThink_Exception('Session.cookieMode can only be "none" "allow" or "only"');
	}
	 ...
	 * @throws RThink_Exception if the value is beyond [0,100]
	 */
	public function setGCProbability($value)
	{
		...
		if($value>=0 && $value<=100)
		{
			...
			throw new RThink_Exception('Session.gcProbability "{value} => '.$value.'" is invalid. It must be an integer between 0 and 100.');
	}
	...
	//------ The following methods enable Session to be CMap-like -----
