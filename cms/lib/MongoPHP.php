...
class MongoPHP{
    ...
    public function __construct($config = array('host'=>'localhost','port'=>27017,'cmd'=>'$')){
        ...
        if (!empty($config['username']) && !empty($config['password'])) {
            $server = sprintf("mongodb://%s:%s%s:%s", $config['username'], $config['password'], $config['host'], $config['port']);
        } else {
    ...
    public function save($sets, $safe=false, $fsync=false){
        ...
        $ret = $this->_getCol()->save($sets,array('safe'=>$safe,'fsync'=>$fsync));
        return $ret;
    }
    ...
    public function count($query=array(),$limit=0,$skip=0){
        return $this->_getCol()->count($query,$limit,$skip);
    }
    ...
    public function fetchOne($query=array(), $fields='_id'){
        $ret = $this->fetchRow($this->_col,$query,array($fields));
    ...
    public function fetchCol($query=array(), $fields='_id'){
        $result = array();
        $list = $this->select($this->_col,$query,array($fields));
        foreach ($list as $row){
            $result[] = $row[$fields];
        }
        return $result;
    ...
    public function fetchAssoc($query=array(), $fields='_id'){
        $result = array();
        $list = $this->select($this->_col,$query);
        foreach ($list as $row){
            $key = $row[$fields];
            $result[][$key] = $row;
        }
    ...
    private function _parseId($arr){
        if(isset($arr['_id'])){
            // 处理in操作中的id
            if (isset($arr['_id']['$in']) && is_array($arr['_id']['$in'])) {
                $ids = array();

                foreach ($arr['_id']['$in'] as $id) {
                    $ids[] = new MongoId($id);
                }
    ...
    private function _getCol(){
            return $this->_db->selectCollection($this->_col);
    }
